<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="使用时间" prop="shiyongshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.shiyongshijian" 
                type="datetime"
                placeholder="使用时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shiyongshijian" label="使用时间" prop="shiyongshijian">
...
export default {
  ...
  computed: {
    zongjiage:{
      get: function () {
        return 1*this.ruleForm.xiaoshijiage*this.ruleForm.shiyongshizhang
      }
    },
  },
  created() {
	this.ruleForm.shiyongshijian = this.getCurDateTime()
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shiyongdingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shiyongdingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
