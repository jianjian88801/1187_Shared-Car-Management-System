<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="汽车名称" prop="qichemingcheng">
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.qichemingcheng" label="汽车名称" prop="qichemingcheng">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="回收时间" prop="huishoushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.huishoushijian" 
                type="datetime"
                placeholder="回收时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.huishoushijian" label="回收时间" prop="huishoushijian">
              <el-input v-model="ruleForm.huishoushijian" 
                placeholder="回收时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.huishoushijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.huishoudidian = true;
          ...
          if(o=='huishoushijian'){
            this.ruleForm.huishoushijian = obj[o];
	    this.ro.huishoushijian = true;
            continue;
          }
        }
      }
    ...
    qichemingchengChange () {
      this.$http({
        ...
        if (data && data.code === 0) {
              if(data.data.qicheleixing){
                this.ruleForm.qicheleixing = data.data.qicheleixing
              }
              if(data.data.pinpai){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `qicheruku/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
